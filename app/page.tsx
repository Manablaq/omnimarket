...
type MarketSort = "newest" | "volume" | "closing";
...
async function callReferenceApi(asset: string): Promise<ReferenceData> {
  const response = await fetch(`/api/omnimarket/reference?asset=${encodeURIComponent(asset)}&interval=1m&limit=60`, { cache: "no-store" });
...
export default function Home() {
  ...
  const visibleMarkets = useMemo(() => snapshots
    ...
    .sort((a, b) => {
      if (marketSort === "volume") return Number(BigInt(b.volumeWei) - BigInt(a.volumeWei));
  ...
  const indicativePayout = selectedPrice > 0 && /^\d+(\.\d{1,18})?$/.test(stakeGen.trim())
    ? Math.round(Number(stakeGen) * 10000 / selectedPrice * 10000) / 10000
    : 0;
  ...
  function changeNetwork(network: NetworkName) {
    setSelectedNetwork(network); setWalletVerified(false); setWalletMenuOpen(false);
    if (wallet) void connectWallet(network);
    else setNotice({ label: `${networkLabel(network)} selected`, detail: network === "testnetBradbury" ? "The OmniMarket contract is deployed here." : "No OmniMarket deployment is configured on Asimov.", tone: network === "testnetBradbury" ? "info" : "warn" });
  }
  ...
  async function submitTrade() {
    ...
    try {
      ...
      setNotice({ label: "GEN position submitted", detail: `Accepted on Bradbury: ${txHash}`, tone: "good" });
  ...
  async function lockMarket() {
    if (!selected) return;
    setBusy("lock");
    try { const txHash = await walletWrite("lock_market", [BigInt(selected.market.market_id)]); setNotice({ label: "Market locked", detail: `Accepted on Bradbury: ${txHash}`, tone: "good" }); await refreshSelected(selected.market.market_id, false); }
  ...
  async function resolveMarket() {
    ...
    try { const txHash = await walletWrite("resolve_market", [BigInt(selected.market.market_id)]); setNotice({ label: "Resolution accepted", detail: `Accepted on Bradbury: ${txHash}`, tone: "good" }); await refreshSelected(selected.market.market_id, false); }
  ...
  async function voidLockedMarket() {
    ...
    try {
      ...
      setNotice({ label: "Market voided", detail: `Settlement timeout accepted on Bradbury: ${txHash}`, tone: "good" });
  ...
  async function claimPosition(marketId = selected?.market.market_id) {
    if (!marketId) return;
    setBusy("claim");
    try { const txHash = await walletWrite("claim_winnings", [BigInt(marketId)]); setNotice({ label: "Payout requested", detail: `GEN payout message accepted: ${txHash}`, tone: "good" }); await refreshPortfolio(wallet); await refreshSelected(marketId, false); }
  ...
  async function claimVoidSeed() {
    ...
    try {
      ...
      setNotice({ label: "Seed recovery requested", detail: `GEN seed refund accepted: ${txHash}`, tone: "good" });
